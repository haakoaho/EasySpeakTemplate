...
document.addEventListener('DOMContentLoaded', () => {
    ...
    generateBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            updateStatus('success', 'Agenda downloaded and forms updated successfully! 🎉');
        } catch (error) {
    ...
    */
    ...
    async function updateForms(meetingData) {
        ...
        const speakers = meetingData.speakers.map(s => s.name).filter(name => name && name !== "TBA");
        
        const evaluators = meetingData.agenda_items
            .filter(item => (item.event.includes("Evaluate speech") || item.event.includes("Table Topics Evaluator")) && item.presenter && item.presenter !== "TBA")
        ...
        const postData = async (url, options) => {
            try {
                ...
                console.warn(`Could not update form at ${url}. This might be expected due to 'no-cors' mode, but check for network errors:`, e);
            }
        };
    ...
    function getStructuredRoles(meetingData) {
        ...
        for (const item of meetingData.agenda_items) {
            ...
            if (!item.role || item.role === 'Break' || !item.presenter || item.presenter.toLowerCase() === 'tba') continue;
